...
\begin{itemize}
    ...
    \item \textbf{Tamaño del lote (Batch Size):} Establecido en 32 imágenes para equilibrar la estabilidad de la estimación del gradiente con las restricciones de memoria de GPU.
    \item \textbf{Épocas de Entrenamiento:} El modelo básico \texttt{CustomCNN} fue entrenado durante 10 épocas, mientras que el modelo \texttt{ResNet-50} fue entrenado durante 5 épocas de ajuste fino (fine-tuning) sobre el clasificador final debido a su rápida convergencia.
    \item \textbf{Comparación de Tasas de Aprendizaje:} Se evaluó la estabilidad y velocidad de convergencia de la arquitectura \texttt{ResNet-50} sintonizando con dos valores de learning rate diferentes: $1 \times 10^{-4}$ y $5 \times 10^{-5}$ durante un intervalo rápido de 3 épocas.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Estabilidad del Optimizador frente a Tasas de Aprendizaje:} Evaluando si el modelo de transferencia es sensible o robusto frente al rango de tasas de aprendizaje adaptativo seleccionadas.
\end{enumerate}
